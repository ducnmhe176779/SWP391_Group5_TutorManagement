...
(function ($) {
    ...
    var ThemeBuilder = function () {
        ...
        var manageImageViewer = function () {
            if (!checkSelectorExistence('.gallery-img')) {
                return;
            }
        ...
        /* Manage Blog Thumbnail */
        var manageBlogThumbnail = function () {
            if (!checkSelectorExistence('.action-box.blog-lg img')) {
                return;
            }

            // Thêm thuộc tính onerror để hiển thị hình ảnh dự phòng
            jQuery('.action-box.blog-lg img').each(function () {
                jQuery(this).attr('onerror', 'this.src="' + window.location.pathname + '/uploads/default_image.jpg"');
            });

            // Áp dụng CSS để kiểm soát kích thước ảnh
            jQuery('.action-box.blog-lg img').css({
                'width': '100%',
                'max-width': '800px',
                'height': '400px',
                'object-fit': 'cover',
            ...
            // Thêm placeholder khi ảnh bị lỗi
            jQuery('.action-box.blog-lg img[onerror]').css({
                'background-color': '#f0f0f0',
                'background-image': 'url("' + window.location.pathname + '/uploads/placeholder.png")',
        ...
        return {
            initialHelper: function () {
                ...
                manageLeftSideMenu();
                manageImageViewer();
                manageBlogThumbnail(); // Thêm hàm mới vào đây
